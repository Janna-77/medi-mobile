...
export default function DoctorAddIndependent() {
    ...
    const handleAgree = async () => {
        ...
        setLoading(true)
        try {
            await api.post('/upgrade/doctor/add-independent', formData)
            setSuccess(true)
        } catch (err) {
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
                <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
                    ...
                    <View style={[styles.card, { backgroundColor: C.container }]}>
                        ...
                        <Text style={[styles.subtitle, { color: C.hint }]}>More information is required to create an independent account</Text>
                            ...
                            {formData.national_id_number.length > 0 && (
                                <Text style={[styles.hint, {
                                    ...
                                    {formData.national_id_number.length < 14
                                        ...
                                        : idErrors?.length === 0 ? '✓ Valid' : `✗ ${idErrors[0]}`}
                                </Text>
                            )}
                        </View>
                        ...
                        <SubmitButton label={loading ? 'Creating...' : 'Create Account'} onPress={handleSubmit} disabled={loading} color={C.border} />
