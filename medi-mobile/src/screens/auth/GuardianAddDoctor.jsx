...
export default function GuardianAddDoctor() {
    const navigation = useNavigation()
    const [formData, setFormData] = useState({
        medical_license_number: '', specialization: '', clinic_name: '', licence_expiry_date: '',
    })
    ...
    const handleAgree = async () => {
        ...
        try {
            ...
            await api.post('/upgrade/guardian/add-doctor', data, { headers: { 'Content-Type': 'multipart/form-data' } })
            setSuccess(true)
        } catch (err) {
            ...
            <SafeAreaView style={[styles.screen, { backgroundColor: C.bg }]}>
                <View style={styles.successWrap}>
                    <View style={[styles.card, { backgroundColor: C.container }]}>
                        ...
                        <Text style={[styles.successSub, { color: C.hint }]}>Your account is pending admin verification.</Text>
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
                <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
                    ...
                    <View style={[styles.card, { backgroundColor: C.container }]}>
                        ...
                        <Text style={[styles.subtitle, { color: C.hint }]}>Please fill in your medical details</Text>

                        <View style={[styles.disclaimer, { borderColor: C.border, backgroundColor: 'rgba(116,13,78,0.08)' }]}>
                        ...
                        <Field label="Medical License Number *" color={C.text}>
                            <TextInput style={[styles.input, { borderColor: C.border, color: C.text }]} value={formData.medical_license_number} onChangeText={set('medical_license_number')} returnKeyType="next" onSubmitEditing={() => specRef.current?.focus()} placeholderTextColor={C.hint} />
                        </Field>
                        ...
                        <Field label="Clinic Name" color={C.text}>
                            <TextInput ref={clinicRef} style={[styles.input, { borderColor: C.border, color: C.text }]} value={formData.clinic_name} onChangeText={set('clinic_name')} returnKeyType="next" onSubmitEditing={() => expiryRef.current?.focus()} placeholderTextColor={C.hint} />
                        </Field>
                        <Field label="Licence Expiry Date" color={C.text}>
                            <TextInput ref={expiryRef} style={[styles.input, { borderColor: C.border, color: C.text }]} value={formData.licence_expiry_date} onChangeText={set('licence_expiry_date')} placeholder="YYYY-MM-DD" placeholderTextColor={C.hint} returnKeyType="done" />
                        </Field>

                        <Field label="Upload Medical Certificate" color={C.text}>
...
const styles = StyleSheet.create({
    ...
    disclaimer: { borderRadius: 10, borderWidth: 1.5, padding: 12 },
